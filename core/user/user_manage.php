              ...
              <form action="" method="POST">
                <select class="form-control custom-select" name="pcode" id="pcode" onchange="this.form.submit()">
                  <?php foreach ($province as $key3 => $value3) { ?>
                    <option value="<?= $key3 ?>" <?= ($pcode == $key3) ? " selected " : "  "; ?>><?= $value3 ?>
            ...
            <?php if ($user["privilege"] > "4") { ?>
              <div class="row mt-3 mb-3">
                <div class="col col-12 btn-group " role="group">
                  <a href="?filter=0" class="btn btn-outline-danger  btn-block"><i class="fas fa-user-times"></i>
                    ผู้ถูกระงับ</a>
                  <a href="?filter=1" class="btn btn-outline-warning  btn-block"><i class="fas fa-hourglass"></i>
                    ผู้รออนุมัติ</a>
                  <a href="?filter=2" class="btn btn-outline-info  btn-block"><i class="fas fa-user-check"></i>
                    ผู้อนุมัติแล้ว</a>
...
//          $mc->check_array($app_option, "app_option");
                    ...
                    <tr class="w3-hover-blue w3-theme-l5">
                            ...
                            <div class="row mt-3">
                              <div class="col col-12 btn-group " role="group">
                                <button type="submit" name="delete"
                                  onclick="return confirm('ยืนยันการลบ <?= $value["pid"] . ' ' . $value["username"] ?>')"
                                  value="2" class="btn btn-danger  btn-block"><i class="fa-solid fa-user-xmark"></i>
                                  ลบผู้ใช้งาน</button>
                              </div>
                            </div>
                          </form>
                      ...
                      <td class="w3-left-align">
                        <?=
                          ...
                          . "<hr class='text-indigo-600 bg-indigo-600'></hr> <b>สิทธิการใช้งาน</b>"
                ...
                <td class="text-left">< ?= $amphoe[$value["acode"]]["pname"] . "<br>" . $amphoe[$value["acode"]]["aname"]; ?></td>-->
                      ...
                      if ($user["privilege"] > "3") {
                        ...
                        ?>
                        <td class="text-center">
                          สถานะการใช้งานปัจจุบัน : <span class="lead">
                            <?= $sm[$smv] ?>
                          </span><br>
                          <?php if ($value["privilege"] !== "9" || $user["privilege"] === "9") { ?>
                            <form action="" method="POST">
                              ...
                              <div class="row mt-3 mb-3">
                                <div class="col col-12 btn-group " role="group">
                                  <button type="submit" name="mem_status" value="2" class="btn btn-outline-info  btn-block"
                                    <?= ($smv === "2") ? " disabled " : "  "; ?>><i class="fas fa-user-check"></i>
                                    อนุมัติการใช้งาน</button>
                                  <button type="submit" name="mem_status" value="1" class="btn btn-outline-warning  btn-block"
                                    <?= ($smv === "1") ? " disabled " : "  "; ?>><i class="fas fa-hourglass"></i>
                                    รอการอนุมัติ</button>
                                  <button type="submit" name="mem_status" value="0" class="btn btn-outline-danger  btn-block"
                                    <?= ($smv === "0") ? " disabled " : "  "; ?>><i class="fas fa-user-times"></i>
                                    ระงับการใช้งาน</button>
                                </div>
                              </div>
                            </form>
                          <?php } ?>
                      ...
                      <td class="text-center">
                        <?php if ($value["password"] !== "e406bb0da445882fb812666cdb3cb47f896ef828b7eb0e34bbcf910603a898e7") { ?>
                          <form action="" method="post">
                            ...
                            <button type="submit" name="submit" class="btn btn-outline-danger me-1 mb-1"
                              onclick="return confirm('ต้องการรีเซ็ตรหัสผ่านของ <?= $value["username"]; ?>');"><i
                                class="fas fa-undo"></i> รีเซ็ตรหัสผ่าน</button>
                          </form>
                          <br>
