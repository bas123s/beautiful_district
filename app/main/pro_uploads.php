...
$sql = "SELECT * from pro_uploads where pcode='" . @$user['pcode'] . "'";
...
<section class="relative  md:py-24 py-16">
    <div class="container mx-auto px-4  relative">
        ...
        <div class="card-body md:text-xl text-xl font-medium text-center">
            <h2 class=" text-center md:text-xl text-xl font-medium ">ที่ทำการปกครองจังหวัด<?= @$user['pname']; ?>
            </h2>
            <h2 class=" text-center md:text-xl text-xl font-medium ">หัวข้อการประเมินหลัก 2. การจัดการความเรียบร้อยของที่ทำการปกครองอำเภอ (5 ส.) และอำนวยความสะดวกแก่ประชาชน</h2>
            ...
            <table class="table table-striped table-responsive" id="example">
                ...
                <tbody>
                    ...
                    /*  foreach ($province as $pcode => $pname) {
                      foreach ($amphoe[$pcode] as $acode => $aname) {
                      วิธีการเรียนกใช้ file respone6 เหมาะสำหรับการทำ fix ไม่เหมาะกับ dochage
                      } */
                    ...
                    foreach (@$data as $key => $value) {
                        ...
                        <tr>
                            ...
                            <td><?= $value['aname']; ?></td>
                            <td>
                                <form id ="form_upload_<?= $i; ?>" method="post" name="form_upload" action="form_pro_upload.php">
                                    <button type="submit"
                                            class="btn btn-primary  md:text-2xl text-2xl text-white font-bold py-2 px-4 border-b-4rounded">อัพโหลดไฟล์ </button>
                                            <?php if ($value['status'] == 2) {
                                                ?>
           <a href="../../../../uploads/upload_file/GEN_file<?=$value['acode']; ?>.pdf"
           class="btn btn-success  md:text-2xl text-2xl text-white font-bold py-2 px-4 border-b-4rounded">ดูรายละเอียด </a> 
           <?php
                                            }
                                 ?>
                                    ...
                                    <input type="hidden" name="aname" value="<?= $value['aname']; ?>">
                                </form>
                            </td>
                        </tr>
                    <?php } ?>
